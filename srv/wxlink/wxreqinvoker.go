...
func (wxqi *WXReqInvoker) SendGetContactRequestForList(userInfoList []string, roomWxIDList []string) (*wechat.GetContactResponse, error) {
	...
	//遍历getContactResp.ContactList将用户添加进db.AddContact
	var contacts []*wechat.ModContact
	contacts = getContactResp.ContactList
	for i := 0; i < len(contacts); i++ {
		db.AddContact(tmpUserInfo.WxId, contacts[i])
	}
