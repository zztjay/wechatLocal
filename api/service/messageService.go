...
func SendMsgXMLService(queryKey string, m model.SendShareCardModel) vo.DTO {
	return checkExIdPerformNoCreateConnect(queryKey, func(wxAccount wxface.IWXAccount, newIWXConnect bool) vo.DTO {
		loginState := wxAccount.GetLoginState()
		...
		resp, err := bizcgi.SendMsgXmlReq(wxAccount, m.ContentType, m.ToUserName, m.Id, m.Nickname, m.Alias, m.Content)
		var result gin.H
		if err != nil {
			result = gin.H{
				"toUserName":    m.ToUserName,
