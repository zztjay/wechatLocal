...
func GetSyncMsgApi(ctx *gin.Context) {
	...
	result := service.GetSync(queryKey, *reqModel)
	ctx.JSON(http.StatusOK, result)
}
...
// GetProfileApi 获取个人资料信息（本地）
func GetProfileLocalApi(ctx *gin.Context) {
	queryKey, isExist := ctx.GetQuery("key")
	if !isExist || strings.Trim(queryKey, "") == "" {
		//确保每次都有Key
		ctx.JSON(http.StatusOK, vo.NewFailUUId(""))
	...
	result := service.GetProfileLocalService(queryKey)
	ctx.JSON(http.StatusOK, result)
}
