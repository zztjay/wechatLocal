...
func setApi_V1(engine *gin.Engine) {
	...
	message := engine.Group(ver + "/message")
	{
		...
		message.POST("/SendMsgXML", controller.SendMsgXMLApi)
	...
	{ //
		...
		user.POST("/GetContactAllList", controller.GetContactAllListApi)
		user.POST("/SyncContactAllList", controller.SyncContactAllListApi)
		...
		user.POST("/GetSyncMsg", controller.GetSyncMsgApi)
		...
		user.GET("/GetProfileLocal", controller.GetProfileLocalApi)
